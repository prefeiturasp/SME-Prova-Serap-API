...
namespace SME.SERAp.Prova.Dados
{
    public static class DapperExtensionMethods
    {
        ...
        public static IEnumerable<TEntity> GetAll<TEntity>(this IDbConnection connection, IDbTransaction dbTransaction = null, bool buffered = true) where TEntity : class
        {
            ...
            try
            {
                var result = Dommel.DommelMapper.GetAll<TEntity>(connection, dbTransaction, buffered);
        ...
        public static async Task<TEntity> GetAsync<TEntity>(this IDbConnection connection, object id) where TEntity : class
        {
            ...
            {
                insightsClient?.TrackDependency("PostgreSQL", "GetAsync", $"{nameof(TEntity)} -> {id} -> erro: {ex.Message}", inicioOperacao, timer.Elapsed, false);
                throw;
            }
        }
