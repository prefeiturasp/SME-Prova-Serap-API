...
SELECT p.prova_legado_id AS prova_serap_id,
   ...
   FROM f_alunos_turma(turma_codigo_eol) fat
   	 inner join aluno a on a.ra = fat.aluno_ra
     JOIN turma t on t.codigo = turma_codigo_eol
     JOIN ue ON t.ue_id = ue.id
     JOIN dre ON ue.dre_id = dre.id
     JOIN prova p ON p.prova_legado_id = prova_serap_id      
     and t.ano_letivo::double precision = date_part('year'::text, p.inicio)
     JOIN prova_ano pa on pa.prova_id = p.id
     and (
